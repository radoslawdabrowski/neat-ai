...
img_background = pygame.transform.scale(pygame.image.load(os.path.join("game/graphics", "bg.png")).convert_alpha(),
                                        (600, 900))
WIN_WIDTH = 600
WIN_HEIGHT = 800
FLOOR = 730
...
def eval_genomes(genomes, config):
    ...
    for genome_id, genome in genomes:
        ...
        nets.append(net)
        birds.append(player.Bird(230, 350))
        ge.append(genome)

    base = ground.Ground(FLOOR)
    pipes = [obstacle.Pipe(700)]
    score = 0
    ...
    while run and len(birds) > 0:
        ...
        if add_pipe:
            ...
            for genome in ge:
                genome.fitness += 5
            pipes.append(obstacle.Pipe(WIN_WIDTH))
...
if __name__ == '__main__':
    ...
    config_path = os.path.join(local_dir, 'algorithm/config.txt')
    run(config_path)
